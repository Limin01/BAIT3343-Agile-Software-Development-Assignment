...
if ($findresult && mysqli_num_rows($findresult) > 0) {
    $results = mysqli_fetch_assoc($findresult);
    $username = $results['username'];
    $oldusername = $results['username'];
    $fname = $results['fname'];
    $lname = $results['lname'];
    $email = $results['email'];
    $image = $results['image'];
                    ...
                    <form action="" method="POST" enctype='multipart/form-data'>
                        <div class="login_form">
                            ...
                            if (isset($_POST['update_profile'])) {
                                ...
                                if (!isset($error)) {
                                    if ($file != "") {
                                        $stmt = mysqli_query($db, "SELECT image FROM  users WHERE email='$email'");
                                        $row = mysqli_fetch_array($stmt);
                                        $deleteimage = $row['image'];
                                        unlink($folder . $deleteimage);
                                        move_uploaded_file($file, $folder . $new_image_name);
                                        mysqli_query($db, "UPDATE users SET image='$new_image_name' WHERE email='$email'");
                                    }
                                    $result = mysqli_query($db, "UPDATE users SET fname='$fname',lname='$lname',username='$username' WHERE email='$email'");
                                    if ($result) {
